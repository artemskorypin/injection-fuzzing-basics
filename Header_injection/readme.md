...
> **одна строка = один payload**, фокус на детекте и базовой разведке.  
...
- Значения из `header_crlf_*` использовать только на стендах/в рамках пентеста. Они подставляются в любые заголовки/поля, где возможен `CRLF` (в том числе в параметры, которые затем попадают в заголовок HTTP-ответа). 
